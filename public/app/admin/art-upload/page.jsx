...
export default function AdminArtUploadPage() {
    ...
    if (!isAdmin)
        return null;
    return (<div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6 flex flex-col items-center">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded shadow w-full max-w-md">
        <label className="block text-gray-700 dark:text-gray-200 mb-2">Pick Your Art:
          <input type="file" accept="image/*" multiple onChange={(e) => setPhotos(Array.from(e.target.files || []))} className="w-full p-2 mb-4 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200"/>
        </label>
        <button onClick={handleUpload} className="w-full px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
          Upload
        </button>
        {previewUrls.length > 0 && (<div className="mt-4">
            <p className="text-green-500 mb-2">Uploaded!</p>
            {previewUrls.map((url, index) => (<img key={index} src={url} alt={`Uploaded Art ${index + 1}`} className="max-w-full h-auto border-2 border-gray-300 dark:border-gray-700 rounded mb-2"/>))}
          </div>)}
        <Link href="/art-show" className="inline-block px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors mt-2">
          View Art
        </Link>
      </div>
      <div className="mt-8 w-full max-w-4xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {artworks.map((art) => (<div key={art.id} className="relative">
              <img src={art.imageUrl} alt="Artwork" className="w-full h-auto border-2 border-gray-300 dark:border-gray-700 rounded"/>
              <button onClick={() => handleDelete(art.id, art.imageUrl)} className="absolute top-1 right-1 px-2 py-1 bg-red-500 text-white rounded hover:bg-red-600">
                X
              </button>
            </div>))}
        </div>
