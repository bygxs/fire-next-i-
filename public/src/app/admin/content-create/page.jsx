...
import { doc, // Reference a specific Firestore doc
...
import { ref, // Create Storage reference
...
export default function AdminContentCreatePage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let photoUrl = "";
            if (photo) {
                const storageRef = ref(storage, `content/${photo.name}`);
                await uploadBytes(storageRef, photo);
                photoUrl = await getDownloadURL(storageRef);
            }
            setContentItems(contentItems.map((item) => item.id === editId ? Object.assign(Object.assign({}, item), { title, body, photoUrl }) : item));
            if (editId) {
                ...
                const newDoc = await addDoc(collection(db, "content"), {
                    ...
                    body,
                    photoUrl,
                    createdBy: (_a = auth.currentUser) === null || _a === void 0 ? void 0 : _a.uid,
    ...
    return (<div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded shadow mb-6">
        <input type="text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full p-2 mb-4 border rounded dark:bg-gray-700 dark:text-gray-200"/>
        <textarea placeholder="Body" value={body} onChange={(e) => setBody(e.target.value)} className="w-full p-2 mb-4 border rounded h-40 dark:bg-gray-700 dark:text-gray-200"/>
        <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-200">
          Upload Photo
          <input type="file" accept="image/*" onChange={(e) => setPhoto(e.target.files ? e.target.files[0] : null)} className="w-full p-2 mb-4 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200"/>
        </label>
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4">
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by title or content..." className="px-4 py-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200 w-full sm:w-1/2"/>
        <label className="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-200">
          Filter by Tag
          <select value={selectedTag} onChange={(e) => setSelectedTag(e.target.value)} className="px-4 py-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200">
      ...
      {filteredItems.length > 0 ? (<ul className="space-y-4">
          {filteredItems.map((item) => (<li key={item.id} className="bg-white dark:bg-gray-800 p-4 rounded shadow flex justify-between">
              <div>
                ...
                {item.photoUrl && (<img src={item.photoUrl} alt={item.title} className="mt-2 max-w-xs"/>)}
              </div>
              <div className="flex gap-2">
                <button onClick={() => handleEdit(item)} className="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600">
                  Edit
                </button>
        ...
        </ul>) : (<p className="text-gray-600 dark:text-gray-400">No content yet.</p>)}
    </div>);
}
