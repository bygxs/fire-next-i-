...
export default function ContentPage() {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                setLoading(true);
                const q = query(collection(db, "content"), orderBy("createdAt", sortOrder), limit(itemsPerPage));
    ...
    const handleNext = async () => {
        if (!lastDoc)
            return;
        try {
            const q = query(collection(db, "content"), orderBy("createdAt", sortOrder), startAfter(lastDoc), limit(itemsPerPage));
            const contentSnapshot = await getDocs(q);
            const contentList = contentSnapshot.docs.map((doc) => {
                const data = doc.data();
                return {
    ...
    const handlePrev = async () => {
        if (page <= 1)
            return;
        try {
            const q = query(collection(db, "content"), orderBy("createdAt", sortOrder), limit(itemsPerPage * (page - 1)));
            const contentSnapshot = await getDocs(q);
    ...
    const toggleExpand = (id) => {
        setExpanded((prev) => (Object.assign(Object.assign({}, prev), { [id]: !prev[id] })));
    };
    /**
    ...
    useEffect(() => {
        const filterItems = () => {
            ...
            if (selectedTag) {
                result = result.filter((item) => { var _a; return (_a = item.tags) === null || _a === void 0 ? void 0 : _a.includes(selectedTag); });
            }
    ...
    return (<div className="min-h-screen bg-gray-100 dark:bg-gray-900 p-6">
      ...
      <Link href="/art-show" className="inline-block px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors mb-4">
        View Art
      </Link>

      {/* Search, Sort, Filter Controls */}
      <div className="mb-6 flex flex-col sm:flex-row gap-4">
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by title or content..." className="px-4 py-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200 w-full sm:w-1/3"/>
        <label htmlFor="sortOrder" className="sr-only">Sort Order</label>
        <select id="sortOrder" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="px-4 py-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200">
          <option value="desc">Newest First</option>
          <option value="asc">Oldest First</option>
        </select>
        <label htmlFor="tagFilter" className="sr-only">Filter by Tag</label>
        <select id="tagFilter" value={selectedTag} onChange={(e) => setSelectedTag(e.target.value)} className="px-4 py-2 border rounded bg-gray-100 dark:bg-gray-700 dark:text-gray-200">
      ...
      {filteredItems.length > 0 ? (<>
          <ul className="space-y-6">
            {filteredItems.map((item) => (<li key={item.id} className="bg-white dark:bg-gray-800 p-4 rounded shadow cursor-pointer" onClick={() => toggleExpand(item.id)}>
                ...
                {item.photoUrl && (<img src={item.photoUrl} alt={item.title} className="mt-2 max-w-full h-auto rounded border-2 border-gray-300 dark:border-gray-700"/>)}
          ...
          <div className="mt-6 flex justify-between">
            <button onClick={handlePrev} disabled={page === 1} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-400">
              Previous
            </button>
            <span className="text-gray-700 dark:text-gray-300">
              Page {page}
            </span>
            <button onClick={handleNext} disabled={contentItems.length < itemsPerPage} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-400">
              Next
            </button>
          </div>
