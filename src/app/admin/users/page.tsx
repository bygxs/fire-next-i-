  ...
  const filteredUsers = users
    .filter((user) => {
      ...
      return (
        ...
        user.tags?.some((tag: string) => tag.toLowerCase().includes(searchTermLower))
      );
    })
    .filter((user) => {
...
export default function AdminPage() {
  ...
  const [newTwitter, setNewTwitter] = useState("");
  const [newFacebook, setNewFacebook] = useState("");
  const [newTags, setNewTags] = useState(""); // State for new user tags
  ...
  const handleCreateUser = async () => {
    ...
    try {
      const id = Math.random().toString(36).slice(2);
      const tagsArray = newTags.split(",").map((tag) => tag.trim()); // Split tags by comma and trim whitespace
      await setDoc(doc(db, "users", id), {
        email: newEmail,
        password: newPassword, // Note: In a real app, hash passwords before storing!
        name: newName,
