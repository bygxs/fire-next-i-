...
export default function ArtShowPage() {
  ...
  const handlePreviousArtwork = () => {
    setCurrentArtworkIndex((prevIndex) => (prevIndex - 1 + artworks.length) % artworks.length); // Decrement index, loop back to the end if at 0
  };

  if (loading) {
    return (
      <div className={`min-h-screen ${darkMode ? "bg-gray-900" : "bg-white"} flex justify-center items-center`}>
        Loading...
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => router.push("/")}
        className="fixed top-4 left-4 p-2 bg-gray-300 text-white rounded z-10"
      >
        <svg
          width="24"
          height="24"
